...
("use strict");
{
  ...
  scoreboardContainer.id = "scoreboard"; // Set the ID for styling
  scoreboardContainer.style.position = "absolute";
  scoreboardContainer.style.bottom = "10px";
  scoreboardContainer.style.left = "50%";
  scoreboardContainer.style.transform = "translateX(-50%)"; // Center the container
  scoreboardContainer.style.background =
    "linear-gradient(to right, #F5BC11, #FF7F50)"; // Yellow background
  scoreboardContainer.style.color = "white"; // Text color
  scoreboardContainer.style.padding = "5px 10px"; // Padding around the text
  scoreboardContainer.style.borderRadius = "30px"; // Rounded corners
  scoreboardContainer.style.zIndex = "1000"; // Ensure it appears above other elements
  scoreboardContainer.style.display = "flex"; // Use flexbox for layout
  scoreboardContainer.style.justifyContent = "space-between"; // Space between items
  scoreboardContainer.style.width = "500px"; // Set a fixed width for the scoreboard
  scoreboardContainer.style.fontFamily = 'Optima, "Segoe UI", sans-serif'; // Set font to Optima with fallbacks
  scoreboardContainer.style.fontWeight = "bold"; // Make text bold
  scoreboardContainer.style.fontSize = "23px"; // Increase font size
  scoreboardContainer.style.textShadow = "5px 5px 5px rgba(0, 0, 0, 0.5)"; // Add text shadow for depth
  scoreboardContainer.style.border = "7px solid #e67e22"; // Set the border to orange
  // Add text border (stroke)
  scoreboardContainer.style.webkitTextStroke = "1px orange"; // Add a black text stroke
  scoreboardContainer.style.textStroke = "1px orange"; // For other browsers (not widely supported)

  const totalCoinsElement = document.createElement("div");
  totalCoinsElement.id = "totalCoins";
  totalCoinsElement.innerText = `TOTAL COINS: ${totalCoinsCollected}`;

  const highScoreElement = document.createElement("div");
  highScoreElement.id = "highScore";
  highScoreElement.innerText = `HIGH SCORE: ${highScore}`;

  // Append elements to the scoreboard container
  scoreboardContainer.appendChild(totalCoinsElement);
  scoreboardContainer.appendChild(highScoreElement);
  document.body.appendChild(scoreboardContainer);

  // Function to update the total coins collected
  function updateTotalCoins(coins) {
    totalCoinsCollected += coins;
    totalCoinsElement.innerText = `Total Coins Collected: ${totalCoinsCollected}`;
  }
  ...
  // Function to simulate the end of a game
  function endGame(coinsCollectedInGame) {
    // Update the total coins collected with the coins from this game
    updateTotalCoins(coinsCollectedInGame);

    // Example: Update high score if the current score is higher
    const currentScore = coinsCollectedInGame; // Replace with actual score logic
    updateHighScore(currentScore);

    // Reset or perform any other end-of-game logic here
    console.log(
      "Game ended. Total coins collected this game: " + coinsCollectedInGame
    );
  }

  // Example usage
  // Simulate collecting coins during the game
  function simulateGame() {
    //const coinsCollectedThisGame = Math.floor(Math.random() * 270); // Random coins collected (0-9)
    console.log(`Coins collected this game: ${coinsCollectedThisGame}`);
    endGame(coinsCollectedThisGame); // Call this function at the end of the game
  }

  // Simulate multiple games
  for (let i = 0; i < 10; i++) {
    simulateGame(); // Simulate 5 games
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (
    ...
    // Example: Update scoreboard values
    // Replace these values with actual game data
    //const totalCoins = 100; // Example value for total coins
    //const highScore = 200; // Example value for high score

    // Update the scoreboard display
    document.getElementById("totalCoins").textContent = totalCoins;
    document.getElementById("highScore").textContent = highScore;
  } else {
    // Hide the scoreboard if not on the home page
...
// Function to update the score when a button is clicked
function updateScore(score) {
  currentScore += score;
  totalCoins += score;

  // Update the total coins and current score in the display
